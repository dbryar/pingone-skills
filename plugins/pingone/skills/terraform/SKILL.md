...
Audit for it on a schedule rather than trusting the plan. Console-created DaVinci connector instances carry a `customerId` field that Terraform-created ones do not, which separates the two populations in one pass over the connector-instances list.

If that list returns 500, do not read it as the API being down. **A PingOne collection endpoint fails whole rather than skipping a member it cannot render**, so one unserialisable record makes the entire collection unavailable while every other row remains readable by ID. Confirmed on `GET /connectorInstances`, which returned 500 for weeks and returned 200 the moment the single bad record was deleted. Walk the IDs you know individually to find the poisoned one; a specific-ID `GET` that 500s while its neighbours return 200 has identified it. Such a record can still be deleted even though it can never be read — `DELETE` returns 204 and the follow-up `GET` returns 404 — but it cannot be imported, so adopting it is impossible and replacing it is the only route.

To reconcile a duplicate: `state rm` the Terraform-managed copy, `import` the console-created one to the same address, apply so every dependent flow re-resolves to the surviving ID, and only then delete the orphan. Deleting before the dependent flows have been re-applied leaves them pointing at an instance that no longer exists.
