...
- **Name a connector instance with the connector's own catalogue name**, which is what Studio names the ones it creates: `GET /environments/{envId}/connectors` returns a `name` per connector (`Variable`, `Functions`, `Flow Analytics`, `Teleport`, `Flow Conductor`, `HTTP`, `Form`, `Error Message`, `Code Snippet`, `User Policy`, `Cookie`, `PingOne`, `PingOne MFA`). A scheme of your own, such as `<connectorId>-instance`, makes a managed instance and a Studio-created one indistinguishable by name in a list that shows both.
