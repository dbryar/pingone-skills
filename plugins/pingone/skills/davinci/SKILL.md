...
- **A connector instance is created the moment a node using it is first authored in Studio**, whether or not one already exists for that connector. Anything authored by hand exists before your infrastructure code hears about it, and a connector opened on the canvas later gets a second instance alongside the one your code already manages. Check for an existing instance before creating one, and re-check periodically afterwards: the flow keeps working, so nothing announces the duplicate. See the `terraform` skill's "Adopting what already exists" for the reconciliation procedure.
- **Name a connector instance with the connector's own catalogue name**, which is what Studio names the ones it creates. A scheme of your own, such as `<connectorId>-instance`, makes a managed instance and a Studio-created one indistinguishable by name in a list that shows both. The id does not predict the name — `nodeConnector` is `Teleport`, `flowConnector` is `Flow Conductor` — so read it rather than guessing: [`reference/connectors.md`](reference/connectors.md) has the table, `GET /environments/{envId}/connectors` is the authority.
